...
async def get_group_icon(group_name: str, db: AsyncSession = Depends(get_db)):
...
async def create_group_icon(icon: GroupIconSchema, db: AsyncSession = Depends(get_db)):
...
async def update_group_icon(group_name: str, icon: GroupIconSchema, db: AsyncSession = Depends(get_db)):
    obj = await db.get(GroupIcon, group_name)
    if not obj:
        raise HTTPException(404, "GroupIcon not found")
    obj.icon = icon.icon
...
async def delete_group_icon(group_name: str, db: AsyncSession = Depends(get_db)):
    obj = await db.get(GroupIcon, group_name)
    if not obj:
        raise HTTPException(404, "GroupIcon not found")
    await db.delete(obj)
