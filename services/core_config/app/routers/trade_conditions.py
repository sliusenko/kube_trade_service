...
async def list_trade_conditions(db: AsyncSession = Depends(get_db)):
    res = await db.execute(select(TradeCondition))
    return res.scalars().all()


@router.get("/{condition_id}", response_model=TradeConditionSchema)
async def get_trade_condition(condition_id: int, db: AsyncSession = Depends(get_db)):
...
async def create_trade_condition(item: TradeConditionSchema, db: AsyncSession = Depends(get_db)):
...
async def update_trade_condition(condition_id: int, item: TradeConditionSchema, db: AsyncSession = Depends(get_db)):
    obj = await db.get(TradeCondition, condition_id)
    if not obj:
        raise HTTPException(404, "TradeCondition not found")
...
async def delete_trade_condition(condition_id: int, db: AsyncSession = Depends(get_db)):
    obj = await db.get(TradeCondition, condition_id)
    if not obj:
        raise HTTPException(404, "TradeCondition not found")
    await db.delete(obj)
