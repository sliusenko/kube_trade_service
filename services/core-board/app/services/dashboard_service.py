...
from app.models import Exchange, User, ServiceAccount, Symbol, FetchLog
...
async def get_dashboard_stats(session: AsyncSession):
    ...
    # 4. Fetch results (успішні/неуспішні)
    q_fetch_overall = await session.execute(
        select(
            FetchLog.type,
            func.sum(func.case((FetchLog.success == True, 1), else_=0)).label("success"),
            func.sum(func.case((FetchLog.success == False, 1), else_=0)).label("fail"),
        ).group_by(FetchLog.type)
    )
    fetch_by_type = [
        {"type": row[0], "success": row[1], "fail": row[2]}
        for row in q_fetch_overall.all()
    ]
